...
PLAYBOOK = {
    "新兴机会": {
        "desc": t("小盘但通道开放 + 新品空间足 — 早期机会",
                  "Small market but open access + ample new-product room — an early-stage opportunity"),
        "actions": [
            t("优先中低评论商品切入（≤200 评论 + ≥4.0 分）",
              "Enter via low-to-mid review listings (<=200 reviews and >=4.0 rating)"),
            t("抢占供需缺口价格带（参考上方价格机会模块）",
              "Target under-supplied price bands (see the Price Gap module above)"),
            t("快速测试新品（小批量 100-300 单试水）", "Test new products fast with small 100-300 unit pilot runs"),
            t("监控窗口是否继续打开（参考上方市场窗口模块）",
              "Monitor whether the window keeps opening (see the Market Window module above)"),
        ],
    },
    "低效开放市场": {
        "desc": t("小盘 + 看似开放 + 新品空间小或不稳 — 弱势 / 噪声市场",
                  "Small market + seemingly open + little or unstable new-product room — a weak / noisy market"),
        "actions": [
...
ALL_ARCHETYPES = [
    ...
    ("成长蓝海",       "Top Pick",   "",                                                False),
    ("新兴机会",       "Top Pick",   t("小盘但通道开放 + 新品空间足",
                                       "Small market but open access + ample new-product room"), True),
    ("高热红海",       "Crowded",    "",                                                False),
    ...
    ("稳态老品市场",   "Watch",      "",                                                False),
    ("大盘冷门",       "Avoid",      "",                                                False),
    ("低效开放市场",   "Avoid",      t("小盘 + 看似开放 + 新品空间小或不稳",
                                       "Small market + seemingly open + little or unstable new-product room"), True),
    ("冷门封闭",       "Avoid",      "",                                                False),
    ("待诊断",         "Avoid",      "",                                                False),
]
...
ARCHETYPE_LABELS = {
    "成熟优质市场": t("成熟优质市场", "Mature Premium Market"),
    "成长蓝海": t("成长蓝海", "Growth Blue Ocean"),
    "新兴机会": t("新兴机会", "Emerging Opportunity"),
    "高成长高波动": t("高成长高波动", "High-Growth High-Volatility"),
    "高热红海": t("高热红海", "Hot Red Ocean"),
    "新品潜力区": t("新品潜力区", "New-Product Potential"),
    "稳定现金流市场": t("稳定现金流市场", "Steady Cash-Flow Market"),
    "高门槛利基": t("高门槛利基", "High-Barrier Niche"),
    "稳态老品市场": t("稳态老品市场", "Steady Legacy Market"),
    "大盘冷门": t("大盘冷门", "Large-but-Cold Market"),
    "低效开放市场": t("低效开放市场", "Inefficient Open Market"),
    "冷门封闭": t("冷门封闭", "Cold & Closed"),
    "待诊断": t("待诊断", "Needs Diagnosis"),
}
...
st.markdown(
    ...
    "🎭 <b>Demo Mode</b> — " + t("节选2类目展示", "Showcasing 2 selected categories") + " "
    "</div>",
...
st.markdown(
    ...
    + t("选择一个类目，查看其业务类型对应的打法建议。",
        "Pick a category to see the playbook for its archetype.")
    + "</div>",
    unsafe_allow_html=True,
)
...
# 模块 1：重点 ASIN
chart_title(f"● {t('重点 ASIN', 'Top Opportunity ASINs')} — {selected}")
top_df, cr3_brands = compute_top_opportunity_asins(selected, top_n=5)
if cr3_brands:
    st.caption(t("注：已排除 CR3 头部品牌：", "Note: CR3 leading brands excluded: ")
               + ", ".join(sorted(cr3_brands)))
if top_df is None or top_df.empty:
    st.warning(t("没有符合条件的候选 ASIN", "No qualifying candidate ASINs"))
else:
    display = top_df.copy()
    display = display[["rank", "brand", "asin", "price_low", "review_count", "rate",
                       "review_gap", "rating_percentile", "product_url"]]
    col_rank = t("榜单排名", "Rank")
    col_brand = t("品牌", "Brand")
    col_price = t("价格 ($)", "Price ($)")
    col_reviews = t("评论数", "Reviews")
    col_rating = t("评分", "Rating")
    col_review_gap = t("评论缺口", "Review Gap")
    col_rating_pct = t("评分百分位", "Rating Percentile")
    col_link = t("链接", "Link")
    display.columns = [col_rank, col_brand, "ASIN", col_price, col_reviews, col_rating,
                       col_review_gap, col_rating_pct, col_link]
    st.dataframe(
        display,
        hide_index=True,
        ...
        column_config={
            ...
            col_review_gap: st.column_config.ProgressColumn(
                min_value=0, max_value=1, format="%.2f",
                help=t("评论数相对类目中位的缺口比例 — 越高表示成长空间越大",
                       "Review count gap vs. category median — higher means more room to grow")),
            col_rating_pct: st.column_config.ProgressColumn(
                min_value=0, max_value=1, format="%.2f",
                help=t("类目内 rating 的排名分位 — 越高表示产品质量越靠前",
                       "Within-category rating percentile — higher means better product quality")),
            col_link: st.column_config.LinkColumn(display_text=t("🔗 demo", "🔗 demo")),
        },
    )
...
# 模块 2：价格机会
chart_title(f"● {t('价格机会', 'Price Gap')} — {selected}")
bands = compute_price_gap(selected)
if bands is None or bands.empty:
    st.warning(t("数据不足，无法分析价格带", "Not enough data to analyze price bands"))
else:
    pg_col1, pg_col2 = st.columns([1.5, 1])
    with pg_col1:
        lbl_supply = t("供给（ASIN 占比）", "Supply (ASIN share)")
        lbl_demand = t("需求（sales_heat 占比）", "Demand (sales-heat share)")
        plot_df = bands.melt(
...
# 模块 3：市场窗口
chart_title(f"● {t('市场窗口', 'Market Window')} — {selected}")
state, signals, daily_df, score = compute_market_window(selected)
if state is None:
    st.warning(t("日度数据不足 14 天，无法计算市场窗口",
                 "Fewer than 14 days of daily data — can't compute the market window"))
else:
    # 注：state_meta 的键（Opening/Stable/Closing）是查找键，保持英文；label/desc 是展示文案，包 t()
    state_meta = {
        "Opening": {"label": t("打开中", "Opening"), "color": "#27ae60", "icon": "↑",
                    "desc": t("窗口正在打开 — 头部松动/新品牌涌入",
                              "Window is opening — leaders loosening / new brands entering")},
        "Stable":  {"label": t("稳定", "Stable"),   "color": "#94a3b8", "icon": "→",
                    "desc": t("结构稳定 — 无显著变化", "Structure is stable — no significant change")},
        "Closing": {"label": t("关闭中", "Closing"), "color": "#e74c3c", "icon": "↓",
                    "desc": t("窗口正在关闭 — 头部固化/品牌固定",
                              "Window is closing — leaders entrenching / brands locked in")},
    }
    sm = state_meta[state]
    st.markdown(
        ...
        f"({t('得分', 'score')} {score:+d}) — {sm['desc']}</span>"
        f"</div>",
        unsafe_allow_html=True,
    )
    ...
    for i, s in enumerate(signals):
        with sig_cols[i]:
            ...
            st.markdown(
                ...
                f"({t('阈值', 'threshold')} ±{s['threshold']:.3f})</div>"
...
chart_title(f"● {t('打法建议', 'Playbook')} — {archetype_label}")
if pb is None:
    st.markdown(
        ...
        f"🔒 <b>{archetype_label}</b> — "
        + t("该业务类型的完整 4 项行动建议在完整版可见",
            "The full set of 4 action items for this archetype is available in the full version")
        + "</div>",
        unsafe_allow_html=True,
    )
else:
    st.markdown(
        ...
        f"<b>{t('特征', 'Profile')}</b>：{pb['desc']}"
        f"</div>",
        unsafe_allow_html=True,
    )
    st.markdown("**" + t("具体行动项：", "Action items:") + "**")
    for i, action in enumerate(pb["actions"], 1):
        st.markdown(f"{i}. {action}")

# 13 archetype 全集概览（5 unlocked + 8 locked）
st.markdown("<div style='height:24px;'></div>", unsafe_allow_html=True)
with st.expander(t("📋 13 业务类型完整框架（2 公开 / 11 加密 — 展开查看）",
                   "📋 Full 13-archetype framework (2 public / 11 locked — expand to view)"), expanded=False):
    st.markdown(
        "<div style='color:#6b7280; font-size:0.8rem; margin-bottom:12px;'>"
        + t("demo 公开 2 个对应当前可选类目的业务类型打法；剩余 11 个仅展示名字 + 策略归属。",
            "The demo unlocks the playbooks for the 2 archetypes matching the selectable categories; "
            "the remaining 11 show only the name and strategy group.")
        + "</div>",
        unsafe_allow_html=True,
    )
    strategy_color = {
